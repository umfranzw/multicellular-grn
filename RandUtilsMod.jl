...
rand_floats(config::Config, start::Float64, stop::Float64, n::Int64=1) = map(r -> start + r * (stop - start), Random.rand(config.rng, n))
rand_enum_val(config::Config, enum::DataType) = Random.rand(config.rng, instances(enum))

rand_decision(config::Config, threshold::Float64) = rand_float(config) < threshold
