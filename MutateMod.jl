...
function mutate_gene(gene::Gene)
    ...
    for site in gene.reg_sites
        mutate_props(gene.config, site, [ProteinPropsMod.ProteinRegAction, ProteinPropsMod.ProteinAppAction])
    end

    #Production sites:
    #target must stay as it is, other attributes can mutate
    for site in gene.prod_sites
        mutate_props(gene.config, site, [ProteinPropsMod.ProteinType, ProteinPropsMod.ProteinRegAction, ProteinPropsMod.ProteinAppAction])
    end
end

function mutate_initial_protein(protein::Protein)
    #type and target must stay the same, others can be mutated
    mutate_props(protein.config, protein.props, [ProteinPropsMod.ProteinRegAction, ProteinPropsMod.ProteinAppAction])
