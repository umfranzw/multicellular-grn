...
function update_fitnesses(pop::Array{Individual, 1}, ea_step::Int64, output_buf::IOBuffer)
    ...
    rs_counts = map(i-> i.reg_sim_info.reg_step_count, pop)
    mean_rs_count = Statistics.mean(rs_counts)
    std_rs_count = Statistics.std(rs_counts, mean=mean_rs_count)
    write(output_buf, @sprintf("mean fitness: %0.5f\n", mean_fit))
    write(output_buf, @sprintf("std fitness: %0.5f\n", std_fit))
    write(output_buf, @sprintf("mean len: %0.5f\n", mean_len))
    write(output_buf, @sprintf("std len: %0.5f\n", std_len))
    write(output_buf, @sprintf("mean rs_count: %0.5f\n", mean_rs_count))
    write(output_buf, @sprintf("std rs_count: %0.5f\n", std_rs_count))
